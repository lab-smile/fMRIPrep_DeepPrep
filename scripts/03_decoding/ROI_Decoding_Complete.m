function ROI_Decoding_Complete(classifier, fold_method, pipeline, confounds, cohort, opts)
% ROI_Decoding_Complete  Unified ROI LSS decoding for SPM/DeepPrep/fMRIPrep.
%
...
%   ROI_Decoding_Complete('fitcsvm','balanced','deepprep',6)
%
% fMRIPrep, 29 confounds, 16 participants, LIBSVM, non-stratified folds:
%   ROI_Decoding_Complete('libsvm','unbalanced','fmriprep',29)
%
% SPM 6-conf, LIBSVM balanced, run BOTH cohorts: all20 and sub16:
%   ROI_Decoding_Complete('libsvm','balanced','spm',6)
%
% SPM 6-conf, fitcsvm unbalanced, only original 16-subject subset:
%   ROI_Decoding_Complete('fitcsvm','unbalanced','spm',6,'sub16')
%
% Optional override structure:
%   opts.data_root = '/orange/ruogu.fang/pateld3/data';
%   opts.spm_root  = '/orange/ruogu.fang/pateld3/data/LSS';
%   opts.out_dir   = '/orange/ruogu.fang/pateld3/data/lss_decoding_results';
%   opts.ncpu      = 16;
%   opts.K         = 10;
%   opts.repeats   = 100;
%   opts.use_parallel = true;
%   ROI_Decoding_Complete('libsvm','balanced','deepprep',6,'sub16',opts)
%
% PARAMETERS
% -------------------------------------------------------------------------
...
%               LIBSVM modes use historical global z-scoring before
%               cross-validation. Use the foldwise_zscore variant to avoid
%               fitting scaling parameters on held-out samples.
...
if strcmp(classifier, 'libsvm')
    assert(exist('svmtrain', 'file') == 2 && exist('svmpredict', 'file') == 2, ...
...
parfor job_idx = 1:size(job_list, 1)
    ...
    try
        acc1 = run_decoder(data_plnt, labels_plnt, classifier, fold_method, K, NUM_REPEATS);
        acc2 = run_decoder(data_upnt, labels_upnt, classifier, fold_method, K, NUM_REPEATS);
        results_cell{job_idx} = struct('subj', subj, 'roi', roi, ...
            'pleasant_vs_neutral', acc1, 'unpleasant_vs_neutral', acc2, ...
            'nvox', sum(valid_voxels), 'n_pl', size(Pl_roi,1), ...
            'n_nt', size(Nt_roi,1), 'n_up', size(Up_roi,1), 'status', 'success');
    catch ME
        results_cell{job_idx} = struct('subj', subj, 'roi', roi, ...
            'status', 'decode_error', 'error_msg', ME.message);
...
%% ========================================================================
function acc = run_decoder(data, labels, classifier, fold_method, k, repeats)
%RUN_DECODER Dispatch to the selected classifier and fold implementation.
if strcmp(classifier, 'libsvm')
    if strcmp(fold_method, 'balanced')
        acc = svm_repeat_libsvm_balanced(data, labels, k, repeats);
    else
        acc = svm_repeat_libsvm_unbalanced(data, labels, k, repeats);
    end
else
    if strcmp(fold_method, 'balanced')
        acc = svm_repeat_fitcsvm_balanced(data, labels, k, repeats);
    else
        acc = svm_repeat_fitcsvm_manual_unbalanced(data, labels, k, repeats);
    end
end
end

%% ----- fitcsvm balanced: SampleBalanced.m methodology -----
function avg_acc = svm_repeat_fitcsvm_balanced(data, labels, k, repeats)
%SVM_REPEAT_FITCSVM_BALANCED Use repeated stratified fitcsvm folds.
[data, labels] = clean_rows(data, labels);
...
function avg_acc = svm_repeat_fitcsvm_manual_unbalanced(data, labels, k, repeats)
...
function avg_acc = svm_repeat_libsvm_balanced(data, labels, k, repeats)
%SVM_REPEAT_LIBSVM_BALANCED Use balanced class chunks with linear LIBSVM.
[data, labels] = clean_rows(data, labels);
[data, labels, ok] = prepare_libsvm_data(data, labels);
if ~ok, avg_acc = NaN; return; end
...
for r = 1:repeats
    ...
    for i = 1:k
        ...
        if numel(unique(Y_train)) < 2 || isempty(X_test), continue; end
        try
            model = svmtrain(Y_train, X_train, '-t 0 -q'); %#ok<SVMTRAIN>
            pred = svmpredict(Y_test, X_test, model, '-q'); %#ok<SVMPREDICT>
            acc(i) = mean(pred == Y_test);
        catch
            continue;
        end
...
function avg_acc = svm_repeat_libsvm_unbalanced(data, labels, k, repeats)
%SVM_REPEAT_LIBSVM_UNBALANCED Use random non-stratified LIBSVM folds.
[data, labels] = clean_rows(data, labels);
[data, labels, ok] = prepare_libsvm_data(data, labels);
if ~ok, avg_acc = NaN; return; end
...
%% ========================================================================
function [data, labels] = clean_rows(data, labels)
%CLEAN_ROWS Drop samples containing NaN features.
valid_rows = ~any(isnan(data), 2);
data = data(valid_rows, :);
labels = labels(valid_rows);
end

function [data, labels, ok] = prepare_libsvm_data(data, labels)
%PREPARE_LIBSVM_DATA Validate classes and apply historical global z-scoring.
ok = false;
unique_labels = unique(labels);
if numel(unique_labels) < 2, return; end
constant_features = std(data, 0, 1) == 0;
data = data(:, ~constant_features);
if isempty(data) || size(data, 2) == 0, return; end
data = zscore(data);
if any(isnan(data(:))) || any(isinf(data(:))), return; end
ok = true;
end
...
if strcmp(pipeline_family, 'spm')
    ...
    data_root = get_opt(opts, 'data_root', '/orange/ruogu.fang/pateld3/data');
