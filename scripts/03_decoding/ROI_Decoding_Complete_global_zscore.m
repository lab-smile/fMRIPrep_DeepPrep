function ROI_Decoding_Complete_global_zscore(classifier, fold_method, pipeline, confounds, cohort, opts)
% ROI_Decoding_Complete_global_zscore  Unified ROI LSS decoding for SPM/DeepPrep/fMRIPrep.
%
...
%   ROI_Decoding_Complete_global_zscore('fitcsvm','balanced','deepprep',6)
%
% fMRIPrep, 29 confounds, 16 participants, LIBSVM, non-stratified folds:
%   ROI_Decoding_Complete_global_zscore('libsvm','unbalanced','fmriprep',29)
%
% SPM 6-conf, LIBSVM balanced, run BOTH cohorts: all20 and sub16:
%   ROI_Decoding_Complete_global_zscore('libsvm','balanced','spm',6)
%
% SPM 6-conf, fitcsvm unbalanced, only original 16-subject subset:
%   ROI_Decoding_Complete_global_zscore('fitcsvm','unbalanced','spm',6,'sub16')
%
...
%   ROI_Decoding_Complete_global_zscore('libsvm','balanced','deepprep',6,'sub16',opts)
%
% PARAMETERS
% -------------------------------------------------------------------------
...
%               LIBSVM modes use global z-scoring before cross-validation,
%               matching the original LIBSVM pipeline behavior.
...
fprintf('=== ROI_Decoding_Complete_global_zscore ===\n');
...
for ci = 1:numel(cohort_list)
    ...
    save_file = fullfile(paths.out_dir, sprintf('DecodingResults_%s_%s_%s_%s_global_zscore.mat', ...
...
metadata.script_name = 'ROI_Decoding_Complete_global_zscore';
...
metadata.libsvm_standardization = 'global zscore before cross-validation';
...
% Dispatch centrally so every cohort/contrast uses the same variant mapping.
if strcmp(classifier, 'libsvm')
...
% A failed ROI should not terminate a large parallel cohort job. Preserve the
% failure as structured metadata so it remains distinguishable from chance.
try
...
%SVM_REPEAT_LIBSVM_BALANCED Use balanced folds after global z-scoring.
[data, labels, clean_info] = clean_rows(data, labels);
[data, labels, clean_info, ok] = standardize_global_libsvm_data(data, labels, clean_info);
...
%SVM_REPEAT_LIBSVM_UNBALANCED Use random folds after global z-scoring.
[data, labels, clean_info] = clean_rows(data, labels);
[data, labels, clean_info, ok] = standardize_global_libsvm_data(data, labels, clean_info);
if ~ok, avg_acc = NaN; return; end
assert_libsvm_on_worker();
...
% Reconstruct diagnostics after an exception without attempting decoding.
clean_info = struct();
clean_info.n_features_initial = size(data, 2);
clean_info.n_samples_initial = size(data, 1);
...
function [data_z, labels, clean_info, ok] = standardize_global_libsvm_data(data, labels, clean_info)
% Global standardization for LIBSVM, matching the original scripts:
% remove globally constant/invalid features, then zscore all samples before
% cross-validation.
ok = false;

if isempty(data) || size(data, 2) == 0 || numel(unique(labels)) < 2
    data_z = [];
    return;
end
...
data_z = bsxfun(@rdivide, bsxfun(@minus, data, mu), sigma);

if any(~isfinite(data_z(:)))
    data_z = [];
    return;
end

clean_info.n_features = size(data_z, 2);
clean_info.n_features_removed = clean_info.n_features_initial - clean_info.n_features;
ok = true;
end

function m = mean_or_nan(x)
% Avoid version-specific dependencies on mean(..., 'omitnan').
valid = ~isnan(x);
...
% Combine hemispheres for ordinary ROIs and aggregate IPS0-IPS5 into the
% single IPS feature set used in the paper.
mask = [];
status = 'success';
if strcmp(roi, 'IPS')
    for kk = 0:5
...
% Atlas filenames encode numeric IDs; the label text maps analysis names to
% those IDs without hard-coding a particular atlas release.
...
% Accept either a family plus confound count or an already-qualified config.
...
% Keep cluster defaults reproducible while allowing local/test overrides.
paths = struct();
if strcmp(pipeline_family, 'spm')
...
% Treat absent and explicitly empty options as requests for the default.
