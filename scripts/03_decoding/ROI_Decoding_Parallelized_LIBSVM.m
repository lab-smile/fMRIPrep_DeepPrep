%% ROI_Decoding_Parallelized_LIBSVM - Balanced linear LIBSVM decoding
% Runs both affective contrasts for every subject/ROI pair. Each repetition
% shuffles classes separately and builds balanced test folds before fitting a
% linear LIBSVM model. The historical implementation z-scores the complete
% ROI dataset before cross-validation; retain that behavior for replication.
%
...
save_file = 'N:\Experimental_Data\Max Lobel\IAPS\fMRI Analysis\Decoding Analysis\Replicating Ke Bo 3B\Output\DecodingResults_LIBSVM_v5.mat';
K = 10;
NUM_REPEATS = 100;
subjects = 1:20;
...
parfor job_idx = 1:size(job_list, 1)
    ...
    % Run SVM
    try
        acc1 = svm_repeat_libsvm(data_plnt, labels_plnt, K, NUM_REPEATS);
        acc2 = svm_repeat_libsvm(data_upnt, labels_upnt, K, NUM_REPEATS);
...
% LIBSVM Function
% ------------------------------
function avg_acc = svm_repeat_libsvm(data, labels, k, repeats)
    ...
    for r = 1:repeats
        ...
        for i = 1:k
            ...
            try
                % LIBSVM: linear kernel, no probability estimates
